...
var require_presenceAdapter = __commonJS({
  "node_modules/@supabase/realtime-js/dist/main/phoenix/presenceAdapter.js"(exports) {
    ...
    var PresenceAdapter = class _PresenceAdapter {
      ...
      static onJoinPayload(key2, currentPresence, newPresence) {
      ...
      static onLeavePayload(key2, currentPresence, leftPresence) {
        const currentPresences = parseCurrentPresences(currentPresence);
  ...
  "node_modules/@supabase/realtime-js/dist/main/RealtimeClient.js"(exports) {
    ...
    function createMemorySessionStorage() {
      ...
      return {
        ...
        getItem(key2) {
          return store.has(key2) ? store.get(key2) : null;
        },
        key(index) {
          var _a;
...
var url = process.env.VITE_SUPABASE_URL || "YOUR_URL";
var key = process.env.VITE_SUPABASE_ANON_KEY || "YOUR_KEY";
if (url === "YOUR_URL") {
...
async function run() {
  ...
  const wibTime = d.getTime() + 7 * 60 * 60 * 1e3;
  ...
  let a = 0, b = 0;
  homeroom?.forEach((h) => {
    if (["S", "I", "A"].includes(h.status)) a++;
  });
  attendance?.forEach((h) => {
    if (["S", "I", "A"].includes(h.status)) b++;
  });
