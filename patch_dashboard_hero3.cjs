...
const regex = /<div className="bg-gradient-to-br from-\\[#f59e0b\\] to-\\[#ea580c\\] rounded-3xl p-6 text-white shadow-lg relative overflow-hidden">[\s\S]*?\{\!isAdmin && \([\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?\)}[\s\S]*?<\/div>/;
...
if(regex.test(code)) {
    ...
    console.log("Regex not matched Dashboard Hero");
}
