      ...
      <div className="print:hidden space-y-6">
        ...
        <div className="bg-slate-100 p-6 rounded-2xl shadow-sm border border-slate-100">
            <div className="grid md:grid-cols-4 gap-4 items-end">
                <div><label className="block text-xs font-bold text-slate-500 mb-1">Kelas</label><select className="bg-white text-slate-800 border-slate-200 dark: dark: w-full border rounded-xl p-3 font-bold focus:ring-2 focus:ring-blue-600 dark: dark:border-slate-600" value={selectedClass} onChange={(e) => setSelectedClass(e.target.value)}><option value="">-- Pilih Kelas --</option>{availableClasses.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
