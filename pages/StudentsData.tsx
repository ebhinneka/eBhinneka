      ...
      <div className="space-y-6">
        ...
        <div className="bg-slate-100 p-5 rounded-3xl shadow-sm border border-slate-100 flex flex-col md:flex-row gap-4 items-center">
            ...
            <div className="w-full md:w-64 relative">
                 ...
                 <select className="w-full pl-11 pr-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-slate-100 text-sm font-bold text-slate-700 appearance-none" value={filterClass} onChange={e => setFilterClass(e.target.value)}>
            ...
            {isModalOpen && (
                ...
                <div className="bg-slate-100 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100 relative animate-fade-in border border-slate-100">
                    ...
                    <div className="p-6">
                             ...
                             <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <select className="w-full border border-slate-200 rounded-xl p-3 bg-slate-100 focus:ring-2 focus:ring-blue-500" value={formData.kelas} onChange={e => setFormData({...formData, kelas: e.target.value})}>{availableClasses.map(c => <option key={c} value={c}>{c}</option>)}</select>
                                    </div>
                                    <div>
                        ...
                        {modalType === 'keluar' && (
                             <div className="space-y-4">
                                 <div><label className="block text-xs font-bold text-slate-500 mb-1.5 ml-1">Pilih Kelas</label><select className="w-full border border-slate-200 rounded-xl p-3 bg-slate-100 focus:ring-2 focus:ring-blue-500" value={mutasiKeluarData.kelas} onChange={e => setMutasiKeluarData({...mutasiKeluarData, kelas: e.target.value, studentId: ''})}><option value="">-- Pilih Kelas --</option>{availableClasses.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                                <div><label className="block text-xs font-bold text-slate-500 mb-1.5 ml-1">Pilih Murid</label><select className="w-full border border-slate-200 rounded-xl p-3 bg-slate-100 disabled:bg-slate-50 disabled:text-slate-400 focus:ring-2 focus:ring-blue-500" value={mutasiKeluarData.studentId} disabled={!mutasiKeluarData.kelas} onChange={e => setMutasiKeluarData({...mutasiKeluarData, studentId: e.target.value})}><option value="">-- Pilih Nama Murid --</option>{studentsForDropdown.map(s => (<option key={s.id} value={s.id}>{s.name} ({s.nisn})</option>))}</select></div>
