  ...
  const MultiSelectDropdown = ({ options, selectedIds, onChange, placeholder }: any) => {
      ...
      return (
          <div className="relative" ref={wrapperRef}>
              <button type="button" onClick={() => setIsOpen(!isOpen)} className="w-full border border-slate-200 dark:border-slate-600 rounded-xl p-3 bg-slate-50 dark:bg-slate-700 text-left flex justify-between items-center focus:ring-2 focus:ring-blue-500"><span className={`truncate text-sm ${selectedIds.length === 0 ? 'text-slate-400 dark:text-slate-500' : 'text-slate-700 dark:text-slate-100 font-bold'}`}>{selectedIds.length === 0 ? placeholder : `${selectedIds.length} Murid Dipilih`}</span><ChevronDown size={16} className={`text-slate-400 transition-transform ${isOpen ? 'rotate-180' : ''}`} /></button>
              {isOpen && (<div className="absolute z-20 w-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg max-h-60 overflow-y-auto p-1 custom-scrollbar">{options.map((opt: any) => { const isSelected = selectedIds.includes(opt.id); return (<div key={opt.id} onClick={() => toggleSelection(opt.id)} className={`flex items-center gap-3 p-2.5 rounded-lg cursor-pointer transition-colors ${isSelected ? 'bg-blue-50 dark:bg-blue-900/30' : 'hover:bg-slate-100 dark:hover:bg-slate-700'}`}><div className={`w-5 h-5 rounded flex items-center justify-center border transition-all ${isSelected ? 'bg-blue-600 border-blue-600' : 'bg-slate-100 border-slate-300'}`}>{isSelected && <Check size={14} className="text-slate-100" strokeWidth={3} />}</div><span className={`text-sm ${isSelected ? 'text-blue-700 dark:text-blue-400 font-bold' : 'text-slate-700 dark:text-slate-300'}`}>{opt.name}</span></div>); })}{options.length === 0 && <div className="p-3 text-center text-xs text-slate-400">Tidak ada murid tersedia (Hadir).</div>}</div>)}
                  ...
                  <React.Fragment key={sch.id}>
                    ...
                    {isSelected && (
                       <div className="animate-fade-in mt-2">
                           {loading ? (
                               <div className="text-center py-8 text-slate-500 text-sm flex flex-col items-center"><Loader2 className="animate-spin mb-2" size={24}/> Mengambil data murid...</div>
                           ) : (
                ...
                {inputMode === 'manual' ? (
                    <div className="w-full max-w-md">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <select className="bg-white text-slate-800 border-slate-200 dark: dark: border p-3 rounded-xl dark:border-slate-600" value={formData.kelas} onChange={(e) => { setFormData({...formData, kelas: e.target.value}); }}>
                                <option value="">- Pilih Kelas -</option>
                                {availableClasses.map(c => <option key={c} value={c}>{c}</option>)}
                            </select>
                            <input 
                                type="text" 
                                className="text-slate-900 dark:text-slate-100 bg-slate-50 dark:bg-slate-800  border p-3 rounded-xl dark:border-slate-600" 
  ...
  return (
    <Layout>
      <div className="max-w-xl mx-auto pb-24">
                ...
                <div className="bg-slate-100 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden transform scale-100 transition-all border border-slate-100 relative animate-fade-in">
                    ...
                    <div className="p-6 space-y-4 bg-slate-50">
                        ...
                        <button onClick={() => handleAssessmentSelect('tugas')} className="w-full flex items-center gap-4 p-4 bg-slate-100 border border-slate-200 rounded-2xl hover:border-blue-600 hover:shadow-lg hover:shadow-blue-300 transition-all group text-left">
                            ...
                            <div><span className="font-bold text-slate-700 block text-sm">Tugas / PR</span><span className="text-xs text-slate-500">Pengumpulan tugas murid.</span></div>
                        </button>
        ...
        {/* MODAL 2: PILIH MURID TIDAK MENGUMPULKAN - TOP ALIGNED */}
            ...
            <div className="fixed inset-0 z-[9999] flex items-start justify-center pt-[calc(env(safe-area-inset-top)+1rem)] sm:p-4 bg-slate-900/50 backdrop-blur-sm transition-all duration-300">
                <div className="bg-slate-100 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform scale-100 transition-all border border-slate-100 relative animate-fade-in flex flex-col max-h-[85vh]">
                    <div className="bg-blue-600 p-5 flex justify-between items-center text-slate-100 flex-shrink-0">
                        <h3 className="font-bold text-lg flex items-center gap-2"><ClipboardList size={20}/> Murid Belum Tuntas/Mengumpulkan</h3>
                    ...
                    <div className="p-4 bg-sky-100 border-b border-blue-300 flex-shrink-0">
                        <p className="text-xs text-slate-900 font-bold">Centang murid yang TIDAK mengumpulkan tugas / Remidi.</p>
                    </div>
                    <div className="flex-1 overflow-y-auto p-2 custom-scrollbar bg-slate-100">
                        ...
                        {getPresentStudents().length === 0 && <div className="text-center p-8 text-slate-400 italic text-sm">Semua murid tidak hadir.</div>}
                    </div>
