        ...
        <div className="space-y-6 animate-fade-in">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex flex-wrap gap-2 items-center bg-slate-100 p-2 rounded-xl border border-slate-200 shadow-sm">
                    <div className="relative"><Search className="absolute left-3 top-2.5 text-slate-400" size={16}/><input type="text" placeholder="Cari Guru..." className="text-slate-900 dark:text-slate-100 bg-slate-50 dark:bg-slate-800  pl-9 pr-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 w-40 dark: dark: dark:border-slate-600" value={hmSearch} onChange={(e) => setHmSearch(e.target.value)}/></div>
                    <div className="h-6 w-px bg-slate-200 mx-1"></div>
                    <select className="text-slate-900 dark:text-slate-100 bg-slate-50 dark:bg-slate-800 py-2 px-3 border border-slate-200 rounded-lg text-sm font-bold dark: dark:border-slate-600" value={selectedMonth} onChange={(e) => setSelectedMonth(Number(e.target.value))}>{monthNames.map((m, i) => <option key={i} value={i}>{m}</option>)}</select>
                    <select className="text-slate-900 dark:text-slate-100 bg-slate-50 dark:bg-slate-800 py-2 px-3 border border-slate-200 rounded-lg text-sm font-bold dark: dark:border-slate-600" value={selectedYear} onChange={(e) => setSelectedYear(Number(e.target.value))}>{[2024, 2025, 2026].map(y => <option key={y} value={y}>{y}</option>)}</select>
                </div>
            </div>
